...
\begin{table*}[t]
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		Busca em Profundidade (DFS) & Não            & 90{,}00           & 13{,}00       & 0{,}216          \\
		Busca em Largura (BFS)      & Não            & 19{,}00           & 26{,}00       & 0{,}332          \\
		Busca de Custo Mínimo (UCS) & Sim            & 17{,}00           & 26{,}00       & 0{,}363          \\
		Busca A*                    & Sim            & 17{,}00           & 19{,}00       & 0{,}254          \\
		\bottomrule
	\end{tabular}
\end{table*}
...
\begin{itemize}[leftmargin=1.5cm]
	\item \textbf{Busca em Profundidade (DFS)}: Apresentou o menor tempo médio de CPU
	      ($0{,}216\text{ ms}$) e a menor média de nós visitados ($13{,}00$). No entanto, esse
	      desempenho computacional aparece por ter encontrado uma solução sub-ótima mais cedo,
	      como vemos que o custo médio da travessia foi de $90{,}00\text{ minutos}$, mais de cinco
	...
	\item \textbf{Busca em Largura (BFS)}: Explorou exatamente $26{,}00\text{ nós}$ na
	      média, com tempo de execução de $0{,}332\text{ ms}$. A BFS cumpre estritamente sua
	      ...
	      $19{,}00\text{ minutos}$, acima do ótimo -- valor que corresponde exatamente ao
	      caminho subótimo de 5 passos detalhado na Seção~\ref{subsec:por-que-bfs-nao-garante-otimo}.
	      Como a BFS é cega em relação ao tempo dos participantes, ela escolhe uma sequência
	      de 5 passos que não emparelha os indivíduos mais lentos de forma estratégica.

	\item \textbf{Busca de Custo Mínimo (UCS)}: Encontrou o custo ótimo de
	      $17{,}00\text{ minutos}$ em 100\% dos ensaios, consumindo $0{,}363\text{ ms}$ e
	...
	\item \textbf{Busca A*}: Também alcançou a solução ótima de
	      $17{,}00\text{ minutos}$, porém visitando apenas $19{,}00\text{ nós}$ em
	      $0{,}254\text{ ms}$ -- menos nós que a UCS e no menor tempo entre os dois métodos
	      ótimos. Essa diferença evidencia o benefício prático da heurística $h_{\max}$: como
	      ela estima corretamente que a pessoa mais lenta remanescente na origem ainda
	      precisará atravessar a ponte, o A* descarta estados cujo custo estimado
	      $f(n) = g(n) + h(n)$ já excede o custo da melhor solução conhecida, evitando expandir
	      parte dos estados que a UCS explora integralmente. Ainda assim, como o espaço de
	      estados do problema com 4 pessoas é pequeno (32 estados no total), a economia
	      proporcionada pela heurística é modesta; em instâncias com mais pessoas, espera-se
	      que essa poda se torne ainda mais significativa.
\end{itemize}
